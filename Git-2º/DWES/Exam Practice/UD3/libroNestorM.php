...
<body>
    ...
    IF ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        empty($_POST['synopsis'])) {
            $errors['empty_field'] = '<-- ALL FIELDS MUST BE FULFILLED';
        }
        ...
        */
        ...
        if (!preg_match($edition_date_regexp, $edition_date)) {
            $errors['edition_date'] = '<-- Error in the edition date';
        }
        ...
        */
        if(!preg_match($ijsbn_regexp,$isbn)) {
        ...
        */
        ...
        if (is_uploaded_file($_FILES['sample']['tmp_name']) == true) {
            $new_path = '.\\sample\\'.$_FILES['sample']['name'];
            if (is_file($new_path) == true) {
                echo 'There is already a file with that name';
                exit();
            }
